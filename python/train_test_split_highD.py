...
data_root = '../dataset/HighD/recorded_trackfiles/'
total_frames = 12
...
def train_test_split_slice(data_set, test_size, random_state):
    ...
    train_data.to_csv('../dataset/HighD/prediction_train/train_data.csv', index=False)
    test_data.to_csv('../dataset/HighD/prediction_test/test_data.csv', index=False)
...
def train_test_split_sampling(data_set, test_size, random_state):
    ...
    rand_max = data_set['frame'].max() - total_frames + 1
    sample_start = random.randint(1, rand_max)
    sample_end = sample_start + total_frames
    used_frame = {}
    test_data = data_set.loc[(data_set.frame == sample_start)]
    used_frame[sample_start] = 1
    for i in range(sample_start+1, sample_end):
        test_data = test_data.append(data_set.loc[(data_set.frame == i)])
        used_frame[i] = 1
    ...
    tmp_i = 0
    max_frame = data_set['frame'].max()
    for i in range(0, max_frame):
        if i not in used_frame.keys():
            train_data = data_set.loc[(data_set.frame == i)]
            tmp_i = i
            break
    for i in range(tmp_i+1, max_frame):
        if i not in used_frame.keys():
            train_data = train_data.append(data_set.loc[(data_set.frame == i)])
...
def load_data(split_file_path_list):
    ...
    last_track_id = data_set['trackId'].max()
    last_frame_id = data_set['frame'].max() + total_frames
    for file_path in split_file_path_list[1:]:
        df = pd.read_csv(file_path)
        df['trackId'] += last_track_id
        df['frame'] += last_frame_id
...
if __name__ == '__main__':
    file_path_list = sorted(glob.glob(os.path.join(data_root, '*.csv')))
