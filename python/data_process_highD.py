...
data_root = '../dataset/HighD/'

history_frames = 6  # 3 second * 2 frame/second
future_frames = 6  # 3 second * 2 frame/second
total_frames = history_frames + future_frames
max_num_object = 50  # maximum number of observed objects is 70
...
total_feature_dimension = 17 + 1  # we add mark "1" to the end of each row to indicate that this row exists
...
def get_frame_instance_dict(pra_file_path):
	...
	Read raw data from files and return a dictionary:
		{frame_id:
			{object_id:
				# 17 features
				[recordingId,object_id,frame_id,trackLifetime,xCenter,yCenter,heading,width,length,xVelocity,yVelocity,xAcceleration,yAcceleration,lonVelocity,latVelocity,lonAcceleration,latAcceleration]
			}
		}
    ...
    with open(pra_file_path, 'r') as reader:
        ...
        now_dict = {}
        for row in content:
            n_dict = now_dict.get(row[2], {})
            n_dict[row[1]] = row
            now_dict[row[2]] = n_dict
    return now_dict
